...
//                    self.mediaView.frame = self.mediaView.frame
    ...
    @nonobjc
    public func presentationIndex(for: UIPageViewController) -> Int {
        Logger.debug("\(logTag) in \(#function)")

        guard let mediaPageViewController = `for` as? MediaPageViewController else {
            owsFail("\(self.logTag) unknown view controller. \(`for`)")
            return 0
        }
